...
[^1]: A plain function `value` is **stored, not called** — it becomes a per-cell display routine, invoked with the `options` object **alone** — `({ row, col, origin }) => { ... }` — and with `this` bound to the drawing context (see [display functions]({{< relref display_fns >}})). To have a function **evaluated per cell** at fill time — a fresh object or a varied tile per cell — tag it: `Quadrille.factory(({ row, col }) => new Object(...))`, which marks the function **in place** and returns that same function.